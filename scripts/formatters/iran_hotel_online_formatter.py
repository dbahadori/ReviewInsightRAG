...
class IranHotelOnlineFormatter:
    ...
    def format_hotel_info_for_faiss(hotel_info: dict) -> Document:
        ...
        text = f" {metadata.get('hotel_name', 'نامشخص')}\n"
        text += f" {content.get('hotel_summary', 'هتل نامشخص با مشخصات نامشخص')}\n"
        text += f" {content.get('about_and_cafe', '')}\n"
        text += f" {content.get('internet_and_parking', 'فاقد اینترنت و فاقد پارکینگ')}\n"
        text += f" {content.get('distance_information', '')}\n"
        text += f" {content.get('faqs', 'سؤالی در مورد هتل موجود نیست')}\n"
        text += f" {content.get('policies', 'هیچ سیاست مشخصی برای هتل تعریف نشده است')}\n"
        text += f" {content.get('hotel_labels', 'برچسبی برای هتل تعریف نشده است')}\n"
        text += f" {content.get('nearby_info', 'هیچ اطلاعات نزدیکی برای هتل نامشخص یافت نشد')}\n"
        text += f" {content.get('club_offers', 'هیچ پیشنهادی از خدمات کلوب ارائه نشده است')}\n"
        text += f" {content.get('near_streets', 'هیچ خیابان نزدیکی برای هتل نامشخص یافت نشد')}\n"
